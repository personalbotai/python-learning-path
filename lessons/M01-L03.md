# Variables, Data Types, and Type System
**ID**: `M01-L03`  
**Type**: lesson  
**Duration**: 30-60 menit  
**Tags**: python, tutorial

## Tujuan Pembelajaran
Learn about dynamic typing, basic types (int, float, str, bool), type conversion, and type hints introduction.

## Materi

### Konsep Dasar
Python menggunakan dynamic typing, artinya tipe data variabel ditentukan secara otomatis berdasarkan nilai yang diberikan, tanpa deklarasi eksplisit. Tipe data dasar di Python meliputi:
- `int`: bilangan bulat (contoh: 42, -10)
- `float`: bilangan desimal (contoh: 3.14, -0.5)
- `str`: teks/string (contoh: "hello", 'world')
- `bool`: boolean True atau False

Type conversion dapat dilakukan dengan fungsi seperti `int()`, `float()`, `str()`, `bool()`. Namun, perlu hati-hati karena konversi yang tidak sesuai dapat menyebabkan error (misal: int("abc")). Python 3.5+ mendukung type hints yang memungkinkan menambahkan anotasi tipe untuk meningkatkan readability dan tooling support, meski tidak memaksa.

### Detail Teknis
- Gunakan `type(variable)` untuk memeriksa tipe data
- Type hints hanya informatif, tidak mengharuskan type checking saat runtime
- Konversi string ke number harus memastikan string tersebut valid
- Python memiliki tipe lain seperti `None`, `list`, `dict`, dll


### Deep Dive: Python Type System & Object Model
Di Python, **semuanya adalah objek** (`everything is an object`). Angka integer, string, fungsi, bahkan class itu sendiri merupakan instance dari metaclass `type`.

1. **Dynamic vs Strong Typing**:
   - **Dynamic**: Tipe data variabel ditentukan saat runtime, bukan saat deklarasi. Anda dapat mengubah `x = 10` menjadi `x = "teks"`.
   - **Strong**: Python tidak melakukan type coercion implisit yang berbahaya. Contoh: `"10" + 5` menghasilkan `TypeError`, bukan `"105"` seperti di JavaScript.

2. **Mutability vs Immutability**:
   - **Immutable**: `int`, `float`, `str`, `bool`, `tuple`, `frozenset`, `bytes`. Saat nilainya dimodifikasi, Python membuat objek baru di memori (`id()` berubah).
   - **Mutable**: `list`, `dict`, `set`, `bytearray`. Objek dapat dimodifikasi di tempat tanpa mengubah alamat memorinya.

3. **Modern Type Hints (PEP 484 & PEP 604)**:
   Mulai Python 3.10+, kita dapat menulis union types dengan operator pipe `|`:
   ```python
   def cari_user(user_id: int | str) -> dict[str, str] | None:
       pass
   ```

### Best Practices & Konvensi Industri
- Terapkan type hints pada seluruh signature fungsi untuk mempermudah autocompletion IDE dan static analysis dengan `mypy` atau `pyright`.
- Gunakan nama variabel yang deskriptif dan mengikuti konvensi `snake_case`.
- Manfaatkan konstanta dengan huruf kapital penuh (`API_TIMEOUT_SECONDS = 30`).

### Common Pitfalls & Antipatterns
- Mengandalkan mutable default argument (misal: `def f(data=[])`), yang akan mempertahankan state antar pemanggilan fungsi! Gunakan `def f(data=None): if data is None: data = []`.
- Membandingkan tipe dengan `type(x) == int` daripada `isinstance(x, int)` yang mendukung hierarki inheritance.

## Latihan

### Soal 1
Buat variabel dengan tipe data yang berbeda (int, float, str, bool) dan tampilkan tipe masing-masing menggunakan fungsi `type()`.

```python
def latihan_1():
    # Tulis kode kamu di sini
    pass

if __name__ == "__main__":
    latihan_1()
```

**Expected Output**:
```
<class 'int'>
<class 'float'>
<class 'str'>
<class 'bool'>
```

### Soal 2
Buat fungsi yang menerima dua angka (string atau number) dan mengembalikan hasil penjumlahan dengan konversi tipe yang tepat. Jika input berupa string, convert ke float terlebih dahulu.

```python
def latihan_2(a, b):
    # Tulis kode kamu di sini
    pass

if __name__ == "__main__":
    print(latihan_2(5, 3))        # Output: 8
    print(latihan_2("5.5", 2.5))  # Output: 8.0
    print(latihan_2("10", "20")) # Output: 30
```

**Expected Output**:
```
8
8.0
30
```

## Referensi
- [Python Official Documentation](https://docs.python.org/3/)
- [Real Python - Python Variables and Data Types](https://realpython.com/python-variables/)
- [W3Schools Python Tutorial](https://www.w3schools.com/python/)

## Next Steps
Setelah menyelesaikan pelajaran ini, lanjutkan ke:
- [M01-L04](M01-L04.md)