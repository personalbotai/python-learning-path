...
### Deep Dive: Mekanisme Iterasi Loop & Klausa `else`
Loop di Python memiliki fitur unik yang sering disalahpahami: **Klausa `else` pada loop**.

```python
for item in daftar_transaksi:
    if item.is_fraudulent():
        print("Fraud terdeteksi! Batalkan.")
        break
else:
    # Dieksekusi HANYA jika loop selesai secara normal TANPA terkena 'break'
    print("Semua transaksi valid dan aman.")
```

### Mengapa `enumerate()` dan `zip()` Sangat Penting?
- `enumerate(iterable, start=0)`: Mengambil indeks dan elemen sekaligus tanpa perlu mengelola counter manual `i += 1`.
- `zip(list_a, list_b)`: Menggabungkan dua atau lebih iterable secara paralel. Gunakan `itertools.zip_longest()` jika panjang keduanya tidak sama.

### Best Practices & Konvensi Industri
- Hindari memodifikasi list saat sedang diiterasi di dalam `for` loop; buat salinan (`list[:]`) atau gunakan list comprehension.
- Gunakan `range(len(arr))` hanya jika Anda benar-benar membutuhkan mutasi indeks; prioritaskan direct iteration `for item in arr:`.
- Batasi while loop tak hingga (`while True`) dengan timeout atau mekanisme stop flag yang eksplisit.

### Common Pitfalls & Antipatterns
- Lupa menambahkan increment di dalam `while` loop yang mengakibatkan infinite loop dan lonjakan CPU.
- Mengira bahwa loop `else` berjalan jika loop tidak pernah dieksekusi; sebenarnya `else` berjalan jika tidak ada `break`.
