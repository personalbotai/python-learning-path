...
### Deep Dive: Operator Logika, Precedence & Walrus Operator (`:=`)
Urutan evaluasi operator logika di Python memiliki prioritas (precedence) dari tertinggi ke terendah:
1. `not` (Negasi)
2. `and` (Konjungsi)
3. `or` (Disjungsi)

Gunakan tanda kurung `( ... )` untuk memperjelas niat kode Anda ketika menggabungkan ekspresi kompleks.

### Assignment Expressions: The Walrus Operator (`:=`) - Python 3.8+
Operator Walrus memungkinkan Anda melakukan assignment variabel di dalam ekspresi kondisional:
```python
# Tanpa Walrus
...
while (data := input_stream.read(1024)):
    process(data)
```

### Best Practices & Konvensi Industri
- Terapkan Hukum De Morgan untuk menyederhanakan ekspresi logika yang berbelit-belit (`not (A or B)` ekuivalen dengan `not A and not B`).
- Gunakan fungsi bawaan `any()` dan `all()` untuk mengevaluasi koleksi boolean secara elegan.

### Common Pitfalls & Antipatterns
- Terjebak dalam urutan precedence: `A or B and C` dievaluasi sebagai `A or (B and C)`, bukan `(A or B) and C`.
